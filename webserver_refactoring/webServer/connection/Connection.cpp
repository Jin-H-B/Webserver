...
void Connection::handleTimeOut()
{
	if (m_clientFdMap.find(currEvent->ident) != m_clientFdMap.end())
	{
		...
		for (it = m_clientFdMap[currEvent->ident].m_server->m_clients.begin(); it != m_clientFdMap[currEvent->ident].m_server->m_clients.end(); ++it)
		{
			if (*it == (int)currEvent->ident)
				break;
		}
		if (it != m_clientFdMap[currEvent->ident].m_server->m_clients.end())
		{
			m_clientFdMap[currEvent->ident].m_server->m_clients.erase(it);
		}
		else
			return;
		delete m_clientFdMap[currEvent->ident].m_responserPtr->m_fileManagerPtr;
		delete m_clientFdMap[currEvent->ident].m_responserPtr;
...
void Connection::handleReadEvent()
{
	...
	if (m_clientFdMap.find(currEvent->ident) != m_clientFdMap.end())
	{
		...
		else if (valRead > 0)
		{
			...
			if (m_clientFdMap[currEvent->ident].status == Res::Complete)
...
void Connection::setNonBlock(int fd)
{
	if (fcntl(fd, F_SETFL, O_NONBLOCK) == FAIL)
		std::cerr << "	ERROR : fcntl()";
}

void Connection::initInfoClient(int clientSocket)
{
