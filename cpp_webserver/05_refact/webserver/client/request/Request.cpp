 ...
 */
void	Request::makeRequest(std::string message)
{
...
void	Request::checkVersion()
{
	if (_version.find(SP) != std::string::npos || _version.size() != 8 || _version.compare(0, 7, "HTTP/1.") != 0)
		return updateStatus(400, ParseError); // Bad Request
	if (_version.compare("HTTP/1.1") != 0 && _version.compare("HTTP/1.0") != 0)
		return updateStatus(505, ParseError); // HTTP Version Not Supported
}
...
{
	_chunkReady = true;
	while (_chunkReady && t_result.pStatus == ParseBody)
	{
		if (_bodyLength == -1)
			getChunkSize();
		if (_bodyLength != -1 && t_result.pStatus == ParseBody)
			getChunkMessage();
	}
}
...
void	Request::getChunkMessage()
{
	...
	{
		if (_buf.size() > 4095 + 1)
			return errorStatus("Chunk message second CRLF error", 400, ParseError);
		_chunkReady = false;
		return ;
	}
